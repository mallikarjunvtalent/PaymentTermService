...
import com.service.BusinessLogic.GeneratePolicyNumber;
...
public class PaymentTermServiceImpl implements PaymentTermService {
	...
	public PaymentTerm saveAll(PaymentTerm paymentTerm) {
		...
		if (paymentTerm.getId() == 0) {
			...
			if (paymentTerm.getDays() == 30) {
				...
				throw new DetailsNotFoundException(
						"please enter valid Days " + paymentTerm.getDays() + " is not valid");
			}
		}
		 ...
		 * paymentTerm.setReminderBeforeDays(4); } else if (paymentTerm.getDays() >= 45)
		 * { paymentTerm.setReminderBeforeDays(5); } else {
		 * paymentTerm.setReminderBeforeDays(2); } }
		 * 
		 */
